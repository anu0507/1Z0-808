...
public class StreamAPIDemo {
	public static List<Student> getTestData() {
		 ...
		 *  Q9. find those Students whose cgpa
		 * is > 8.0?
		 * 
...
//			 Q3. filter those students who studies in 3rd year and belongs to "Bihar"?
		 ...
		 */
	/////	Q10. find those Students whose age is between 20 to 25?
		getTestData().stream(). distinct(). filter(s-> { return s.getDob().equals(getTestData());})
...
//			Q5. group Students by branch and their semester?
//			Q6. sort Students by name?
	///*	((Iterable<Student>) getTestData().stream().sorted().collect(toList())).forEach(System.out::println);
		// Q7. find the Student whose cgpa is maximum?
		
		//Q9. find those Students whose cgpa is > 8.0?
	
		getTestData().stream().filter(s->{ return s.getCgpa()>8.0;}).collect(Collectors.toList())
		.forEach(System.out::println);
		////Q11. sort Students by cgpa (descending order) ? 
		List<Student> collect1 = getTestData().stream().sorted(Comparator.comparing(Student::getCgpa).reversed())
